...
public class Klasifikasi {
  ...
  public static Klasifikasi build(BuildParams[] params) throws Exception {
    if (instance == null) {
      ...
      for (BuildParams data : params) {
        ...
        ClientAuth auth = requestClient.request(RequestMethod.POST, requestTokenUrl, null, null, clientData, ClientAuth.class);

        Map<String, String> headers = new HashMap<>();
        headers.put("Authorization", String.format("Bearer %s", auth.getAuth().getToken()));
        String activeClienturl = String.format("%s/api/v1/auth/activeClient", url);
        ClientModel model = requestClient.request(RequestMethod.GET, activeClienturl, headers, null, null, ClientModel.class);

        modelMapping.put(model.getModel().getPublicId(), new ClientData(auth.getAuth(), model.getModel()));
  ...
  public Map<String, ClientData> getModels() {
    return modelMapping;
  }
